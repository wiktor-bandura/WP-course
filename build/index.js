...
class MyNotes {
  ...
  async deleteNote(e) {
    const thisNote = this.findNearestParentLi(e.target);
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default()["delete"](themeData.root_url + "/wp-json/wp/v2/note/" + thisNote.getAttribute("data-id"));
  ...
  async updateNote(e) {
    ...
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default().post(themeData.root_url + "/wp-json/wp/v2/note/" + thisNote.getAttribute("data-id"), ourUpdatedPost);
      this.makeNoteReadOnly(thisNote);
    } catch (e) {
      console.log("Sorry");
    }
  ...
  async createNote() {
    ...
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default().post(themeData.root_url + "/wp-json/wp/v2/note/", ourNewPost);
...
class Search {
  ...
  keyPressDispatcher = e => {
    if (e.keyCode === 83 && this.isOverlayOpen === false && document.activeElement !== document.querySelector('input') && document.activeElement !== document.querySelector('textarea')) this.openOverlay();
    if (e.keyCode === 27 && this.isOverlayOpen === true) this.closeOverlay();
  };
  typicLogic = () => {
  ...
  getResults = async () => {
    const result = await fetch(`${themeData.root_url}/wp-json/university/search?term=${this.searchInput.value}`).then(res => res.json()).then(result => {
      this.searchResults.innerHTML = `
                    <div class="row">
                        <div class="one-third">
